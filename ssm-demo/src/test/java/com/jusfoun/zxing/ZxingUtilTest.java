...
//		writeImgToFile(content, file);
	...
	public void writeColors() throws FileNotFoundException, IOException, WriterException, NotFoundException {
		...
		for (Colors color1 : values) {
			for (Colors color2 : values) {
				...
				rgb2 = color2.getRGB();
				File file = new File(
						"D:/logos/colorQcode/" + String.format("%05d", i++) + "_" + color1 + "_" + color2 + ".png");
	...
	public void writeImgToFileWithLogo() throws FileNotFoundException, IOException, WriterException, NotFoundException {
		for (int i = 0; i <= 101; i++) {
			...
			writeImgToFileWithLogo(content, 320, 320, DEF_CHARACTER_SET, DEF_FORMAT, Colors.random().getRGB(),
					Colors.random().getRGB(), logoFile, Position.RIGHT_BUTTOM, qCodeFile);
		}
	}
	...
	public void markImageByIcon() {
		...
		makeImageByIcon(logoFile.getAbsolutePath(), qCodeFile.getAbsolutePath(), "d:/logos/qcode/" + 1111 + ".png",
				null);
	}
}
